...
not parse `lumo`'s flags, render prompts, or know anything about
...
2. **Locate the real binary.** Either:
   ...
   - download `cli_<version>_<os>_<arch>.{tar.gz,zip}` and
     ...
     siblings intact** — never extract just the `lumo`/`lumo.exe`
...
3. **Exec, don't shell out through a subshell.** Replace the wrapper
   process with `lumo` where the platform supports it (`execve` on
...
4. **Forward argv and exit code exactly.** Whatever arguments the user
   passed to the wrapper, pass to `lumo` unchanged; exit with
   `lumo`'s own exit code.
...
already ships these as siblings of `lumo` (ADR-0006). If a wrapper
extracts only `lumo` and discards the rest, `lumo plugins
list` finds nothing and `lumo new` fails with
...
but traceable to) the `lumo` version it launches — e.g. an npm
package `2.0.0` launching `lumo v0.1.1` should say so in its
`README`/`CHANGELOG`, so a user filing an issue can tell which `lumo`
release they actually hit.

## `go install` — pipeline-built binaries carry the fallback; `go install` itself doesn't yet

`go install github.com/intruder0007/Lumo/cli@latest` compiles only the
...
no `LUMO_PLUGIN_DIRS` workaround.
...
`distribution/<ecosystem>/README.md` for each one's status. **One is
published**: npm (`lumo-cli`, ADR-0015), published
automatically by the release pipeline; the rest are not yet published —
publishing is a separate, explicitly-confirmed step per ecosystem. The
Go path (`go install`) is handled for pipeline-built binaries by the
embedded fallback (ADR-0012); `go install` itself awaits submodule tags
and build-time staging (see the section above). This contract is a
Stable public API — see `docs/architecture/api-compatibility.md`.
